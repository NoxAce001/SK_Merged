...
import fs from 'fs';
import EventBox from '../../models/Achievement/EventBox.model.js';
...
const getAllImages = async (req, res) => {
  try {
    const images = await EventBox.find().sort({ order: 1 });
    res.json(images);
  } catch (error) {
...
const getPublishedImages = async (req, res) => {
  try {
    const images = await EventBox.find({ isPublished: true }).sort({ order: 1 });
    res.json(images);
  } catch (error) {
...
const validateAndStoreImages = async (req, res) => {
  try {
    const existingCount = await EventBox.countDocuments();
...
const publishWithUpload = async (req, res) => {
  try {
    ...
    await EventBox.insertMany(newImages);

    const allImages = await EventBox.find().sort({ order: 1 });
...
const deleteImage = async (req, res) => {
  try {
    const image = await EventBox.findById(req.params.id);
    ...
    await EventBox.findByIdAndDelete(req.params.id);

    const remainingImages = await EventBox.find().sort({ order: 1 });
    const updatePromises = remainingImages.map((img, index) => {
      return EventBox.findByIdAndUpdate(img._id, { order: index });
    });
...
const reorderImages = async (req, res) => {
  try {
    ...
    const image = await EventBox.findById(imageId);

    if (!image) {
      return res.status(404).json({ message: 'Image not found' });
    }

    const adjacentImage = await EventBox.findOne({
...
const publishImages = async (req, res) => {
  try {
    await EventBox.updateMany({}, { isPublished: true });
